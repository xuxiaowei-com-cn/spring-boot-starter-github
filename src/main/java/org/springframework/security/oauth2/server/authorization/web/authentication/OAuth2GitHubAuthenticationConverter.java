...
import org.springframework.security.oauth2.server.authorization.authentication.OAuth2GitHubAuthenticationToken;
...
/**
 * GitHub OAuth2 用于验证授权授予的 {@link OAuth2GitHubAuthenticationToken}。
 *
 * @author xuxiaowei
 * @since Joe Grandja
 * @since 0.0.1
...
public class OAuth2GitHubAuthenticationConverter implements AuthenticationConverter {
	...
	public Authentication convert(HttpServletRequest request) {
		...
		if (!OAuth2GitHubAuthenticationToken.GITHUB.getValue().equals(grantType)) {
			return null;
		}
		...
		// appid (REQUIRED)
		String appid = parameters.getFirst(OAuth2GitHubParameterNames.APPID);

		if (!StringUtils.hasText(appid) || parameters.get(OAuth2GitHubParameterNames.APPID).size() != 1) {
			OAuth2EndpointUtils.throwError(OAuth2ErrorCodes.INVALID_REQUEST, OAuth2GitHubParameterNames.APPID,
					OAuth2GitHubEndpointUtils.AUTH_CODE2SESSION_URI);
		}
		...
		parameters.forEach((key, value) -> {
			if (!key.equals(OAuth2ParameterNames.GRANT_TYPE) && !key.equals(OAuth2ParameterNames.CLIENT_ID)
					...
					&& !key.equals(OAuth2ParameterNames.CLIENT_SECRET) && !key.equals(OAuth2GitHubParameterNames.APPID)
					&& !key.equals(OAuth2ParameterNames.SCOPE) && !OAuth2GitHubParameterNames.REMOTE_ADDRESS.equals(key)
					&& !OAuth2GitHubParameterNames.SESSION_ID.equals(key)
					&& !OAuth2GitHubParameterNames.BINDING.equals(key)) {
				additionalParameters.put(key, value.get(0));
			}
		});

		String remoteAddress = request.getParameter(OAuth2GitHubParameterNames.REMOTE_ADDRESS);
		String sessionId = request.getParameter(OAuth2GitHubParameterNames.SESSION_ID);

		return new OAuth2GitHubAuthenticationToken(clientPrincipal, additionalParameters, appid, code, scope,
